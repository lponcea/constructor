...
export class VersionService {
  ...
  protected convertDateFromClient(version: IVersion): IVersion {
    const copy: IVersion = Object.assign({}, version, {
      fechaVersion: version.fechaVersion && version.fechaVersion.isValid() ? version.fechaVersion.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((version: IVersion) => {
        version.fechaVersion = version.fechaVersion ? moment(version.fechaVersion) : undefined;
      });
    }
    return res;
  }
